...
#else
unsigned int emu_screen_w = SCREEN_W_DEFAULT;
unsigned int emu_screen_h = SCREEN_H_DEFAULT;
unsigned int emu_screen_bpp = SCREEN_BPP_DEFAULT;
unsigned int emu_screen_pitch = SCREEN_PITCH_DEFAULT;
#endif
...
{
  LOG_ENTER();
  LOGI("CK : gp2x_change_res(%d,%d, %d)", w,h, bpp);
  ...
  setScreenSize(w, h);
#else
  //LOGI("updateScreenSize  is %dx%d (%d)", w,h, bpp);
  updateScreenSize(w,h,bpp, screenBuffer);

  //LOGD("HAHA, I sleep 3 seconds here to monitor crash issue");
  //sleep(3);
  //LOGD("sleep 3 seconds done");
